...
import "./telegram-bg-style.css"

const ChatListBody: React.FC = () => {
    const { data } = useChatListContext();

    return (
        <div className=" telegram-bg-style flex-1 overflow-y-auto p-4 bg-gray-50 ltr">
                    ...
                    >
                        <div className={`${isSent ? 'text-right' : 'text-left'} w-10/12 lg:w-1/2`}>
                            ...
                            <div className="flex items-center mb-1">
                                ...
                                <div className="text-xs text-gray-500">{item.user_name} • {item.department_name}</div>
                            </div>
                                ...
                                <div className="mt-2 space-y-2">
                                    {item.files?.map((file, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {isPhoto ? (
                                                    <img src={href} alt={file.fileName} className="w-32 h-32 object-cover rounded-lg" />
                                                ) : (
